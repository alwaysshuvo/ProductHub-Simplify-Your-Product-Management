...
export default function ProductDetails({ product }) {
  ...
  const safeImg = (url) => (url || "").replace("i.ibb.co.com", "i.ibb.co");
  const [mainImage, setMainImage] = useState(safeImg(product.images?.[0]));
  ...
  const averageRating = ratingsArray.length
    ? ratingsArray.reduce((sum, item) => sum + (item?.rating || 0), 0) / ratingsArray.length
    : 0;
    ...
    <div className="flex max-lg:flex-col gap-12">
      <div className="flex max-sm:flex-col-reverse gap-3">
        <div className="flex sm:flex-col gap-3">
          {product.images?.map((image, index) => (
            <div key={index}
              onClick={() => setMainImage(safeImg(image))}
              className="bg-slate-100 flex items-center justify-center size-26 rounded-lg group cursor-pointer">
              <Image src={safeImg(image)} className="group-hover:scale-103 group-active:scale-95 transition" alt="" width={45} height={45} />
            </div>
          ))}
        </div>

        <div className="flex justify-center items-center h-100 sm:size-113 bg-slate-100 rounded-lg">
          <Image src={safeImg(mainImage)} alt="" width={250} height={250} />
        </div>
      </div>
      ...
      <div className="flex-1">
        ...
        <div className='flex items-center mt-2'>
          ...
          <p className="text-sm ml-3 text-slate-500">{ratingsArray.length} Reviews</p>
        </div>
